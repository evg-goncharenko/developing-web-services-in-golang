...
func getRemoteResource() error {
	...
	if err != nil {
		return errors.Wrap(err, "resource error") // .Wrap - добавление к ошибки доп. информацию
	}
	return nil
}
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		// .Cause(err) - причина ошибки, .(type) - можем узнать тип ошибки (например, string)
		switch err := errors.Cause(err).(type) {
		case *url.Error: // Error - struct{Op, URL, Err}
