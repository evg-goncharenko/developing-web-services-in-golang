...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	err := decoder.Decode(msg, r.URL.Query()) // запись в структуру msg из Query() параметров запроса
	if err != nil {
		fmt.Println(err)
	...
	_, err = govalidator.ValidateStruct(msg) // передаем структуру msg с уже заполненными полями

	if err != nil {

		// .(govalidator.Errors) - преобразование ошибки к нужному типу
		if allErrs, ok := err.(govalidator.Errors); ok {
			for _, fld := range allErrs.Errors() { // идем по всем ошибкам
